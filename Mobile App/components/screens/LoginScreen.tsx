...
const LoginScreen: React.FC<LoginScreenProps> = ({ onProceed }) => {
    ...
    const showCustomMessage = (message: string, triggerId: string) => {
        if (messageTriggerIdRef.current === triggerId) {
            console.log(`Message blocked: triggerId=${triggerId}, message=${message}`);
            return;
        }

        console.log(`Showing message: triggerId=${triggerId}, message=${message}`);
        ...
        messageTimeoutRef.current = setTimeout(() => {
            ...
            messageTimeoutRef.current = null;
            console.log(`Message cleared: triggerId=${triggerId}, message=${message}`);
        }, 3000);
    };
                    ...
                    <View style={styles.inputSection}>
                        <View style={styles.inputWithIconContainer}>
                            <TextInput
                                ...
                                value={email}
                                editable={!isAnyLoading} // Only disable during loading, not after email sent
                                onChangeText={(text) => {
                                    if (!isAnyLoading) {
                                        // If the new text contains .com followed by any characters, prevent it
                                        const comIndex = text.indexOf('.com');
                                        if (comIndex !== -1 && text.length > comIndex + 4) {
                                            return; // Prevent adding characters after .com
                                        }
                                        setEmail(text);
                                    }
                                }}
                        ...
                        <View style={styles.inputWithIconContainer}>
                            <TextInput
                                ...
                                onChangeText={(text) => {
                                    if (!isAnyLoading && text.length <= 10) {
                            ...
                            <TouchableOpacity
                                ...
                                disabled={!phone || !validatePhone(phone) || phoneTimerActive || sendingPhoneOtp || isAnyLoading}
                            >
                                {sendingPhoneOtp ? (
                                    <ActivityIndicator size="small" color="#ff8653" />
                                ) : (
