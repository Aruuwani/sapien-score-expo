...
export const signupWithPassword = async (phone_number: string, email: string, password: string, agreedTerms: boolean = true, workEmail?: string) => {
  try {
    const response = await apiClient.post('auth/register', { phone_number, email, password, agreedTerms, workEmail });
    return response.data;
  } catch (error: any) {
    if (axios.isAxiosError(error) && error.code === 'ECONNABORTED') {
      console.log('error', 'Network Error');
